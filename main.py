...
for index,img in enumerate(encoded_imgs):                    # Tensor aus jedem Bild der Datenbank aufrufen
...
# Vektor aus numpy Array im normalen Array speichern,damit wir Komm Bibliothek benutzen koennen
    ...
    for i in range(0, len(img)):
        ...
        if img_arr[i] >= 1:                 # Array nur aus 0 und 1
            img_arr[i] = 1
    img_arr_1 = QPSK.modulate(img_arr)        # Modulation
    img_arr_2 = awgn(img_arr_1)                 # AWGN im Channel addieren
    img_arr_3 = QPSK.demodulate(img_arr_2)      # Demodulation
    img = img_arr_3.reshape((7, 7, 8))        # Array im 7*7*8 Tensor umwandeln

    if index == 0:
        """
        img_arr_2 = np.real(img_arr_2) + np.imag(img_arr_2)
        t = np.array([])
        for i in np.arange(0, 195, 1):
            t = np.insert(t, len(t), i)
        plt.plot(t, img_arr_2[0:195])
        plt.legend(["AWGN"], loc='upper right')
        plt.show()
        """
